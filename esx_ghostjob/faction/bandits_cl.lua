...
local blips = {
  ...
  {title="~p~Quartier ghost", colour=83, id=84, x = 104.67, y = -1938.46, z = 20.8}
  }
  Citizen.CreateThread(function()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'item' then
        TriggerServerEvent('esx_ghost:buyItem')
      end

      if data.current.value == 'item1' then
        TriggerServerEvent('esx_ghost:buyItem1')
      end

      if data.current.value == 'item2' then
        TriggerServerEvent('esx_ghost:buyItem2')
      end
...
function OpenGetStocksBanditsMenu()
  ...
  ESX.TriggerServerCallback('esx_ghost:getStockItemsBandits', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_ghost:getStockItemsBandits', itemName, count)
            end
...
ESX.TriggerServerCallback('esx_ghost:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_ghost:putStockItemsBandits', itemName, count)
            end
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_ghost:giveWeaponBandits', weapon,  1000)
      end,
      function(data, menu)
...
function OpenGetWeaponBanditsMenu(station)
  ...
  ESX.TriggerServerCallback('esx_ghost:getArmoryWeaponsBandits', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_ghost:removeArmoryWeaponBandits', function()
...
function OpenPutWeaponBanditsMenu(station)
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_ghost:addArmoryWeaponBandits', function()
  ...
  ESX.TriggerServerCallback('esx_ghost:getArmoryWeaponsBandits', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_ghost:buyBandits', function(hasEnoughMoney)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'formol' then
          menu.close()
          TriggerServerEvent('esx_ghost:startHarvestBandits')
        end
      end,
      function(data, menu)
        menu.close()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'puce' then
          menu.close()
          TriggerServerEvent('esx_ghost:startHarvest2Bandits')
        end
      end,
      function(data, menu)
        menu.close()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value == 'spark' then
          menu.close()
          TriggerServerEvent('esx_ghost:startHarvest3Bandits')
        end

        if data.current.value == 'tesla' then
          menu.close()
          TriggerServerEvent('esx_ghost:startHarvest4Bandits')
        end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'sedatif' then
          menu.close()
          TriggerServerEvent('esx_ghost:startCraftBandits')
        end

        if data.current.value == 'cryptedphone' then
          menu.close()
          TriggerServerEvent('esx_ghost:startCraft2Bandits')
        end

        if data.current.value == 'lightbreak' then
          menu.close()
          TriggerServerEvent('esx_ghost:startCraft3Bandits')
        end
...
AddEventHandler('esx_ghost:hasEnteredMarkerBandits', function(zone, station, part, partNum)
...
AddEventHandler('esx_ghost:hasExitedMarkerBandits', function(zone, station, part, partNum)

  if zone == 'HarvestBandits' then
    TriggerServerEvent('esx_ghost:stopHarvestBandits')
  end

  if zone == 'HarvestBandits2' then
    TriggerServerEvent('esx_ghost:stopHarvest2Bandits')
  end  

  if zone == 'HarvestBandits3' then
    TriggerServerEvent('esx_ghost:stopHarvest3Bandits')
    TriggerServerEvent('esx_ghost:stopHarvest4Bandits')
  end
  
  if zone == 'CraftBandits' then
    TriggerServerEvent('esx_ghost:stopCraftBandits')
    TriggerServerEvent('esx_ghost:stopCraft2Bandits')
    TriggerServerEvent('esx_ghost:stopCraft3Bandits')
  end
  ...
  while true do
    ...
    if PlayerData.job2 ~= nil and PlayerData.job2.name == 'ghost' then
      ...
      if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastZone ~= currentZone or LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) ) then
        ...
        then
          TriggerEvent('esx_ghost:hasExitedMarkerBandits', LastZone, LastStation, LastPart, LastPartNum)
          hasExited = true
        end
        ...
        TriggerEvent('esx_ghost:hasEnteredMarkerBandits', currentZone, currentStation, currentPart, currentPartNum)
      end
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job2 ~= nil and PlayerData.job2.name == 'ghost' and (GetGameTimer() - GUI.Time) > 300 then
...
AddEventHandler('esx_ghost:teleportMarkers', function(position)
  ...
  -- while true do
    ...
    -- if PlayerData.job2 ~= nil and PlayerData.job2.name == 'ghost' then
        ...
        -- if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          -- TriggerEvent('esx_ghost:teleportMarkers', position)
        -- end
...
function OpenGetBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_ghost:getBlackMoneySociety', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.type == 'item_weapon' then
          menu.close()
          TriggerServerEvent('esx_ghost:getItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              if quantity == nil then
                ...
                menu.close()
                TriggerServerEvent('esx_ghost:getItem', data.current.type, data.current.value, quantity)
                ESX.SetTimeout(300, function()
                  OpenGetBlackMoney()
                end)
              end
...
function OpenPutBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_ghost:getPlayerInventory2', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_ghost:putItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              TriggerServerEvent('esx_ghost:putItem', data.current.type, data.current.value, tonumber(data2.value))
